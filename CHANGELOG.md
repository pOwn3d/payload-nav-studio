...
holes. **Nothing here fixes a vulnerability and nothing here is urgent.** It is for teams who have
an accessibility audit to answer (EAA / RGAA, WCAG 2.2), who have watched the sidebar take a whole
admin page down with it, or who still hold preference rows written before 0.17.0 shut the two write
paths. No breaking change, no schema change, no migration.

### Added

- **A render boundary around the two components this plugin mounts inside somebody else's panel, and it is the mount points that make it worth doing.** `AdminNav` is injected into `beforeNavLinks`, so it renders on *every* page of the admin panel, and Payload mounts it straight from the import map — the plugin never controls its parent and cannot be handed an ancestor boundary, which is why the boundary has to live inside the exported module. Until now one throw anywhere in the sidebar — an icon name the registry does not know, a stored layout shaped in a way the 0.17.0 sanitizer did not anticipate, a Payload provider that moved — took the entire page down, with the "Customize" link needed to repair it sitting inside the tree that had just died. The exported `AdminNav` is now a thin wrapper around `AdminNavInner` behind a boundary with `fallback={null}`: on failure the plugin's sidebar simply disappears, Payload's own navigation rendered next to it keeps working, and the error goes to the console named after its mount point. `/admin/nav-customizer` gets the same boundary with the built-in notice instead of `null` — a `role="alert"` panel with a retry button, drawn in `--theme-elevation-*` tokens so it stays readable in dark mode — because that page has nothing else to show and a blank screen tells the integrator nothing. The caught error is never put on screen, only in the console: a server-side message carries paths and implementation detail.

- **What the boundary does not cover, said out loud in the README.** React catches render errors, lifecycle methods and constructors — nothing else. The sidebar's own fetches (`/preferences`, `/default-nav`, `/badges`) reject inside async effects, which go straight past any boundary; they stay guarded by the `try` / `catch` and `.catch()` already at each call site, and that is the right place for them. `NavCustomizerView` itself is a *server* component: a client boundary cannot wrap it, since Payload imports it from the import map and renders it before any client tree exists. Its one new failure point — resolving this plugin's i18n keys through the host's `req.i18n` — is guarded by a `try` / `catch` returning the English string, so a host that replaced `i18n.translations` wholesale gets a working error screen rather than a crash on the screen meant to handle crashes.

- **`npx admin-nav-audit-preferences`, a remediation CLI, published as a `bin`.** 0.17.0 shut two write paths and deliberately did not clean up what had already gone through them; this is the sweep. It is **report-only by default** and writes nothing until `--fix` is passed. It reports `foreign-owner` and `no-owner` (rows whose `user` points at no document in the admin auth collection, or at nothing at all — deleted by `--fix`), `duplicate-owner` (**never touched automatically**: with two rows for one owner, deleting the wrong twin destroys a real layout, so a human decides), `layout-rejected`, `layout-filtered`, `layout-invalid` and `collapsed-invalid`. It finds the collection by its four-field signature rather than by the default slug, so a host that renamed `preferencesSlug` is audited instead of silently reported as clean, and it reads the owner collection off `user.relationTo` rather than assuming `users`. It judges rows by importing the plugin's own `dist/utils/navLayoutValidation.js`, so what it calls broken is exactly what the sidebar refuses to render. It re-execs itself through the host's `payload run` — the only way to load `.env` and a TypeScript `payload.config.ts` — passing its flags through an env var, because that CLI rebuilds `process.argv` from positional arguments only and would swallow `--fix` on the way. `--collection=<slug>`, `--batch=<n>`, `PAYLOAD_CONFIG_PATH` and `--help` are documented in the README and in `--help` itself.

- **README: a *Database and updates* section and an *Upgrading* section.** The first states plainly that this plugin adds a collection to your config but **does not own your schema**, that Payload gives a plugin no way to ship migrations (`payload migrate` reads one directory, the host app's `payload.db.migrationDir`), and what the host-side workflow therefore is: `push` in development, `migrate:create` then `migrate` in production, never `push` there. The second answers the question that follows — **no release of this plugin has ever changed the schema**; `admin-nav-preferences` has carried the same four fields (`user`, `navLayout`, `collapsedGroups`, `version`) since 0.12.1, and every release from now on states whether that is still true. It also carries the 0.16.x → 0.17.0 and 0.17.x → 0.18.0 upgrade notes, including which release left data behind and which did not.

- **56 more vitest tests (134 → 190), in three new suites plus one extended.** 14 on the boundary's contract — that a caught error never replays the children (the mistake that makes a boundary unmount the whole root), that `fallback={null}` renders nothing rather than falling through to the notice, that a reset remounts the subtree instead of only clearing a flag, that the error message never reaches the screen. 4 pinning `AdminNav`'s wrapper and the `fallback={null}` it is given, both invisible from the outside until something crashes in production. 5 on the stylesheet, including one that diffs the selectors of `src/styles/admin-nav.css` against the copy inlined in `StyleInjector.tsx` — the two had already drifted once, which is how a fix ships as fixed and behaves as broken. 30 on the audit script's pure helpers. 3 more on the customizer view, covering the boundary it now wraps and the i18n fallback.

- Two translation keys, `viewCrashed` and `retry`, in `en` and `fr` and in `PluginAdminNavTranslationKeys`.

### Fixed

- **18 controls in the customizer announced to a screen reader as unnamed.** Nine are now wired to a real `<label>` through `useId()` and `htmlFor` — the group title and group id, the item label and URL, and the three fields of the sub-item editor — with `useId()` rather than literal ids because the customizer can mount two editors at once and duplicate ids break the very association they exist to create. In multi-language mode, where the single input is replaced by one per language, the shared label points at the first of them rather than at an id nothing renders, and the language code that used to be a decorative `<span>` is now the `<label>` of its own input. The other nine carry an `aria-label`, because no `<label>` could name them: the two search fields (a placeholder is not a name — it disappears exactly when the user needs it announced), the visually hidden import file input (which announced as "file upload"), the four icon-only buttons of the sub-item list (they carried a `title`, which several screen readers ignore), each tile of the icon picker (whose entire content is a decorative `<svg>`, now `aria-hidden` and `focusable="false"`), and the icon-picker toggle — a `<button>`, which `htmlFor` cannot target at all, so its caption became a `<span>` and the button names itself with its current value included, since for a colour its only content is a bare dot.

- **The jump-to field had no visible focus indicator that met WCAG 2.2 SC 2.4.11.** The input clears its own outline, and the only remaining cue was the wrapper's `:focus-within` ring — a 1 px border plus a 1 px shadow at 30 % of the accent colour, nowhere near the 3:1 contrast a focus indicator has to reach. A solid 2 px outline is now drawn on the field itself, under `:focus-visible` so a mouse click does not paint a ring. Applied to **both** copies of the stylesheet — `src/styles/admin-nav.css` and the one inlined in `StyleInjector.tsx`, which is the one the sidebar actually injects — and a test now fails if their selector sets diverge again.

- **The two modals could only be closed with a mouse.** Clicking the backdrop dismissed them; the keyboard had no equivalent. `Escape` now closes both, which is the parity the backdrop click was missing.

- **Seven buttons inside the editors had no `type`, so they defaulted to `submit`.** Cancel and Save in both editors, the icon-picker toggle and its Close button, and each icon tile. None of them sits in a `<form>` today, which is the only reason this was invisible; a host wrapping the customizer in one would have had Enter submit it.

### Changed

- **A crash in the sidebar now looks different, on purpose.** Where the admin page used to go down, the plugin's sidebar disappears, Payload's own navigation keeps working, and `[admin-nav] AdminNav crashed while rendering:` appears in the console with the error and the component stack. Silent degradation is the deliberate choice for a component that renders on every page — a permanent red panel across the sidebar would be worse than the missing feature, and means nothing to anyone who is not the integrator. **If a sidebar goes missing after this upgrade, the console is where the reason is.** The customizer view keeps a visible notice for the opposite reason: that page has nothing else to render.

- **The modal containers no longer carry an `onClick`.** Both editors used the overlay-plus-`stopPropagation` pattern, which makes a plain `<div>` read as a control to assistive technology. The backdrop now closes only on its own clicks (`event.target === event.currentTarget`) and is marked `role="presentation"`; the inner container is `role="dialog"` with `aria-labelledby` pointing at its heading. Dismissal behaves exactly as before. **`aria-modal` is deliberately not claimed**: nothing confines Tab to the dialog subtree yet, and telling assistive technology the rest of the page is inert while it is still reachable by keyboard is worse than saying nothing. A focus trap is the remaining gap, and it is a real one.

- **The caption above the icon picker is a `<span>`, not a `<label>`.** `htmlFor` only targets form controls and the trigger is a `<button>`. The class already sets `display: block`, so nothing moves on screen.

- `ErrorBoundary` is internal: it is built as a standalone `dist` entry with its own `"use client"` directive — both build passes run with `bundle: false`, so a wrapper shared by the client entry and the RSC view has to be an entry of its own or the emitted relative import points at a file that was never written — but it is not re-exported from `/client` and there is no deep import path to it. `pnpm verify:dist` now asserts that file exists and carries the directive, so a build that drops it fails instead of publishing a sidebar whose boundary resolves to nothing.

- **The published tarball now contains `scripts/`**, because the audit tool ships as a `bin`. `scripts/__tests__` is excluded. The vitest `include` glob was widened to `scripts/__tests__/**/*.test.mjs` for the same reason: a published artefact is covered like the rest of the package.
